...
let rule_set = [
  { target: res.get.numeric, rules: ["numeric [[value]]"] },

  { target: ops.get.incrementor, rules: ["@or(increment|add) [[value]]"] },
  ...
  { id: "last", target: ops.get.array.last, rules: ["?(get) last"] },

  { id: "first", target: ops.get.array.first, rules: ["?(get) first"] },

  { target: () => console.log("boii"), rules: ["boi"] },

  { target: x => console.log(x.target), rules: ["say [[target]]"] },

  { id: "foo", target: ops.get.incrementor({ value: 29 }), rules: ["foo"] },

  {
    target: it["store_variable"],
    rules: [
      "store [[value]] @or(in|into|as) [[name]]",
      "[[name]] equals [[value]]",
...

  {
    ...
    rules: ["what is ?(the value of) [[name]]", "output [[name]]"],
    parsers: { name: iu.nato_parser }
  }
];
